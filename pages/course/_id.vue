      ...
      <div>
        <article class="c-v-pic-wrap" style="height: 357px;">
          <section class="p-h-video-box" id="videoPlay">
            <img height="357px" width="100%" :src="courseInfo.cover" :alt="courseInfo.title" class="dis c-v-pic" />
          </section>
        </article>
        ...
        <div>
          ...
          <section class>
            <section class="question-list lh-bj-list pr">
              <ul class="pr10">
                <li v-for="(comment,index) in commonetData.items" v-bind:key="index">
                  <aside class="noter-pic">
          ...
          <div class="paging">
            ...
            <a
              :class="{undisable: !commonetData.hasPrevious}"
              href="#"
              title="首页"
            ...
            <a
              ...
              href="#"
              title="前一页"
              @click.prevent="gotoPage(commonetData.current-1)"
            >&lt;</a>
            <a
              v-for="page in commonetData.pages"
              :key="page"
              :class="{current: commonetData.current == page, undisable: commonetData.current == page}"
              :title="'第'+page+'页'"
              href="#"
              @click.prevent="gotoPage(page)"
            >{{ page }}</a>
            <a
              :class="{undisable: !commonetData.hasNext}"
              href="#"
              title="后一页"
              @click.prevent="gotoPage(commonetData.current+1)"
            >&gt;</a>
            <a
              ...
              href="#"
              title="末页"
              @click.prevent="gotoPage(commonetData.pages)"
            >末</a>
            <div class="clear" />
          </div>
          <!-- 公共分页 结束 -->
...
import commentApi from "@/api/common";
  ...
  methods: {
    ...
    initComment() {
      commentApi
        ...
        .then(response => {
          this.commonetData = response.data.data;
        });
    },
    addComment() {
    ...
    //立即购买生成订单
    createOrder(){
        orderApi.creatrOrder(this.courseId).then(response=>{
              //订单创建成功，跳转到订单页面
